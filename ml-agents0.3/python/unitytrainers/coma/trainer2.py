...
class MAPPOTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            self.target = MAPPOModel(env.brains[brain_name],
                                  lr=float(trainer_parameters['learning_rate']),
                                  h_size=int(trainer_parameters['hidden_units']),
                                  epsilon=float(trainer_parameters['epsilon']),
                                  beta=float(trainer_parameters['beta']),
    ...
    def add_experiences(self, curr_all_info: AllBrainInfo, next_all_info: AllBrainInfo, take_action_outputs, all_actions):
        ...
        other_actions = [[0] for brain_name, action in all_actions.items() if brain_name != self.brain_name]
        agent_action = [[0] for brain_name, action in all_actions.items() if brain_name == self.brain_name]
        ...
        for k in range(num_epoch):
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                feed_dict = {self.model.batch_size: n_sequences,
                             ...
                             self.model.agent_action: np.zeros((end-start, 1))}#np.array(_buffer['actions'][start:end]).reshape([-1, 1])}
                ...
                v_loss, p_loss, _, _, value,values, agent_action, other_actions, returns, advantage = self.sess.run(
                    [self.model.value_loss, self.model.policy_loss,
                     self.model.update_batch, self.model.update_value, self.model.value, self.model.values,
                     self.model.agent_action, self.model.other_actions, self.model.returns_holder, self.model.advantage], feed_dict=feed_dict)
...
def get_co_adv(value_estimates, action_values, action_probs, lambd=0.95):
    ...
    """
    # advantages = []
    # for values, action_value, action_prob in zip(value_estimates, action_values, action_probs):
    #     #idx = list(range(action[0])) + list(range(action[0]+1, len(policy)))
    #     #other_action_probs = policy[idx]
    #     #other_action_values = values[0][idx]
    #     advantage = (action_value - np.dot(action_prob, values[0]))
    #     advantages.append(advantage)
    #advantages = discount_rewards(r=np.array(advantages), gamma=0.99)
    #print(list(advantages))
    #print(action_values)
    #print(action_probs, value_estimates)
