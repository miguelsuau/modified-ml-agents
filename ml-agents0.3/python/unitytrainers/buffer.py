...
class Buffer(dict):
    ...
    def delete_entries_update_buffer(self, n):
        """
        Deletes entries in update buffer to leave room for new experiences
        :param n: The number of entries to delete
        """
        #idx = random.sample(range(len(update_buffer['actions'])), n)
        for key in self.update_buffer.keys():
            self.update_buffer[key] = self.update_buffer[key][n:]
