...
class DQNTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        super(DQNTrainer, self).__init__(sess, env, brain_name, trainer_parameters, training)
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            self.main = DQNModel(env.brains[brain_name],
                                  ...
                                  m_size=self.m_size)
            self.target = DQNModel(env.brains[brain_name],
                                  lr=float(trainer_parameters['learning_rate']),
                                  h_size=int(trainer_parameters['hidden_units']),
                                  epsilon_start=float(trainer_parameters['epsilon_start']),
                                  epsilon_end=float(trainer_parameters['epsilon_end']),
                ...
                if not stored_info.local_done[idx]:
                    if self.use_observations:
                        for i, _ in enumerate(stored_info.visual_observations):
                            ...
                            self.training_buffer[agent_id]['next_observations%d' % i].append(next_info.visual_observations[i][next_idx])
                    if self.use_states:
                    ...
                    actions = stored_take_action_outputs[self.main.random_action]
                    self.training_buffer[agent_id]['actions'].append(actions[idx])
                    self.training_buffer[agent_id]['done'].append(next_info.local_done[next_idx])
    ...
    def process_experiences(self, all_info: AllBrainInfo):
        ...
        for l in range(len(info.agents)):
            ...
            agent_actions = self.training_buffer[agent_id]['actions']
            if (info.local_done[l] and len(agent_actions) > 0):
                self.training_buffer.append_replay_memory(agent_id, replay_memory_size=self.trainer_parameters['replay_memory_size'])
                #print("update buffer size " + str(len(self.training_buffer.update_buffer['actions'])))
    ...
    def sample(self, batch_size):
        ...
        """
        update_batch = {}
        idx = np.random.choice(len(self.training_buffer.update_buffer['actions']), batch_size)
        for key, values in self.training_buffer.update_buffer.items():
            if len(values) > batch_size:
                update_batch[key] = np.array(values)[idx]
        return update_batch
    ...
    def update_model(self):
        ...
        """
        update_batch = self.sample(self.trainer_parameters['batch_size'])
        feed_dict1 = {}
        feed_dict2 = {}
        feed_dict3 = {}
        ...
        if self.use_states:
            if self.brain.vector_observation_space_type == "continuous":
                feed_dict1[self.main.vector_in] = np.array(
                    update_batch['next_states']).reshape(
                    [-1, self.brain.vector_observation_space_size * self.brain.num_stacked_vector_observations])
                feed_dict2[self.target.vector_in] = np.array(
                    update_batch['next_states']).reshape(
                    [-1, self.brain.vector_observation_space_size * self.brain.num_stacked_vector_observations])
                feed_dict3[self.main.vector_in] = np.array(
                    update_batch['states']).reshape(
                    [-1, self.brain.vector_observation_space_size * self.brain.num_stacked_vector_observations])
            else:
                feed_dict1[self.main.vector_in] = np.array(
                    update_batch['next_states']).reshape([-1, self.brain.num_stacked_vector_observations])
                feed_dict2[self.target.vector_in] = np.array(
                    update_batch['next_states']).reshape([-1, self.brain.num_stacked_vector_observations])
                feed_dict3[self.main.vector_in] = np.array(
                    update_batch['states']).reshape([-1, self.brain.num_stacked_vector_observations])
        if self.use_observations:
            for i, _ in enumerate(self.main.visual_in):
                _obs = np.array(update_batch['next_observations%d' % i])
                (_batch, _seq, _w, _h, _c) = _obs.shape
                feed_dict1[self.main.visual_in[i]] = _obs.reshape([-1, _w, _h, _c])
                feed_dict2[self.target.visual_in[i]] = _obs.reshape([-1, _w, _h, _c])
                _obs = np.array(update_batch['observations%d' % i])
                (_batch, _seq, _w, _h, _c) = _obs.shape
        ...
        inverse_done = np.invert(update_batch['done'])
        targets = update_batch['rewards'] + self.trainer_parameters['gamma']*Q2_values*inverse_done
        feed_dict3[self.main.targets] = targets
        feed_dict3[self.main.actions] = update_batch['actions'][:,0]
    ...
    def write_summary(self, lesson_number):
        ...
        if (self.get_step % self.trainer_parameters['summary_freq'] == 0 and self.get_step != 0 and
            ...
            print(len(self.stats['cumulative_reward']))
